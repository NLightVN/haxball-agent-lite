"""
play_a3.3.py — Watch A2.3_3 3v3 Self-Play
...
3v3 layout:
    RED  team = agent (agents[0]) + 2 teammates (current_model clone)
    BLUE team = 3 opponents (opponent_policies)
"""
...
# ── Configuration ──────────────────────────────────────────────────────────────
AGENT_MODEL_PATH    = "models/1v1 oriented/a1.1_checkpoints/snapshot_15000000.zip"  # Agent + đồng đội
OPPONENT_MODEL_PATH = "models/2v2 oriented/a3_1_checkpoints/snapshot_2750000.zip"   # Đối thủ (có thể dùng model cũ hơn)
...
from training.env import HaxballCurriculumEnv, POLE_R, _dist_to_goal_segment
...
def draw_players(screen, env, sr):
    ...
    for i, ag in enumerate(env.agents):
        ...
        team = env.agents_team[i] if i < len(env.agents_team) else (1 if i < 3 else 2)
        color = C_RED if team == 1 else C_BLUE
        ...
        lbl = FONT_SM.render(player_labels.get(i, str(i)), True, (255, 255, 255))
...
def draw_panel(screen, env, step_cnt, ep_reward, step_reward, episode, result_str, paused):
    ...
    right_lines = [
        ...
        f"goal_y: {env.goal_y:.0f}  |  A2.3_3  3v3",
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    print(f"[A2.3_3] Loading agent model  : {AGENT_MODEL_PATH}")
    agent_model = PPO.load(AGENT_MODEL_PATH, device="cpu")

    print(f"[A2.3_3] Loading opponent model: {OPPONENT_MODEL_PATH}")
    opp_model = PPO.load(OPPONENT_MODEL_PATH, device="cpu")

    pygame.init()
    screen = pygame.display.set_mode((WIN_W, WIN_H))
    pygame.display.set_caption("Haxball — A2.3_3 Play (3v3)")
    clock = pygame.time.Clock()
    init_fonts()

    field_rect = pygame.Rect(5, PANEL_H + 5, WIN_W - 10, FIELD_H)

    # A2.3_3: phase='A2.3_3', fixed_mode='3v3' → luôn chơi 3v3
    env = HaxballCurriculumEnv(phase="A2.3_3", fixed_mode="3v3")
    env.forced_opponent_type = "Trained"
    env.opponent_type = "Trained"

    # Đồng đội dùng cùng model với agent (như lúc train)
    env.current_model = agent_model
    env.opponent_deterministic = DETERMINISTIC

    # 3 đối thủ dùng opponent_policies
    env.opponent_policies = [opp_model, opp_model, opp_model]
