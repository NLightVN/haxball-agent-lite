"""
play_a2.2_1.py — Watch A2.2_1 Dynamic Play
...
Dynamic A2.2_1 layout:
    Matches randomly alternate between 1v1 and 2v1 
    based on the A2.2_1 curriculum probabilities.
"""
...
# ── Configuration ──────────────────────────────────────────────────────────────
AGENT_MODEL_PATH    = "models/2v2 oriented/a2_2_1_checkpoints/snapshot_2500000.zip"  # Agent chính
TEAMMATE_MODEL_PATH = "models/2v2 oriented/a2_2_1_checkpoints/snapshot_1000000.zip"  # Đồng đội
OPPONENT_MODEL_PATH = "models/2v2 oriented/a2_1_1_checkpoints/snapshot_2750000.zip"  # Đối thủ
...
def draw_panel(screen, env, step_cnt, ep_reward, step_reward, episode, result_str, paused):
    ...
    right_lines = [
        ...
        f"goal_y: {env.goal_y:.0f}  |  A2.2_1  {mode_str}",
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    print(f"[A2.2_1] Loading agent model  : {AGENT_MODEL_PATH}")
    agent_model = PPO.load(AGENT_MODEL_PATH, device="cpu")

    print(f"[A2.2_1] Loading teammate model: {TEAMMATE_MODEL_PATH}")
    teammate_model = PPO.load(TEAMMATE_MODEL_PATH, device="cpu")

    print(f"[A2.2_1] Loading opponent model: {OPPONENT_MODEL_PATH}")
    opp_model = PPO.load(OPPONENT_MODEL_PATH, device="cpu")

    pygame.init()
    screen = pygame.display.set_mode((WIN_W, WIN_H))
    pygame.display.set_caption("Haxball — A2.2_1 Play (Dynamic 1v1/2v1)")
    clock = pygame.time.Clock()
    init_fonts()

    field_rect = pygame.Rect(5, PANEL_H + 5, WIN_W - 10, FIELD_H)

    # A2.2_1: phase='A2.2_1', fixed_mode=None → Tự động chọn 1v1, 2v1
    env = HaxballCurriculumEnv(phase="A2.2_1", fixed_mode=None)
    env.forced_opponent_type = "Trained"
    env.opponent_type = "Trained"

    # Đồng đội dùng model riêng biệt
    env.current_model = agent_model
    env.teammate_policy = teammate_model
    env.opponent_deterministic = DETERMINISTIC

    # Tối đa 1 đối thủ trong A2.2_1 (1v1 hoặc 2v1)
    env.opponent_policies = [opp_model]
