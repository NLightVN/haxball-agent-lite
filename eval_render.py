...
from training.env import HaxballCurriculumEnv, DIR_MAP, KICK_STR
...
def parse_args():
    ...
    p.add_argument("--a1-model",   required=True,  help="Path to A1 model .zip")
    p.add_argument("--a0-model",   default=None,   help="Path to A0 model .zip (used as opponent)")
    p.add_argument("--opponent",   default=None,
                   help="Force opponent type: Defender | Attacker | Hybrid | Follower | Trained | random | human")
    p.add_argument("--goal",       default=65.0,   type=float,
                   help="Goal half-height in physics units (default: 65)")
...
def main():
    ...
    print(f"Loading A1 model: {args.a1_model}")
    a1_model = PPO.load(args.a1_model, device="cpu")
    ...
    if args.a0_model:
        env.a0_model_path = args.a0_model
    ...
    if args.opponent:
        ...
        if opp_lower == "human":
            ...
            env.forced_opponent_type = args.opponent

    is_human_opp = args.opponent and args.opponent.lower() == "human"
    ...
    def do_reset():
        obs, _ = env.reset()
        env.goal_y = args.goal   # override curriculum goal size
        return obs
    ...
    while running:
        ...
        if now - last_step_t >= STEP_INTERVAL:
            ...
            action, _ = a1_model.predict(obs, deterministic=args.deterministic)
