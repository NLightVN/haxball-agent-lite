...
class OpponentPoolA3:
    ...
    Convention (nhat quan voi train_a1.py):
    ...
    def update_result(self, path: str, agent_won: bool):
        ...
        wins = so lan SNAPSHOT thang (nhat quan voi train_a1.py).
        """
...
if __name__ == "__main__":
    ...
    a1_1_snaps = sorted(
        ...
        key=lambda f: int(os.path.basename(f).replace("a1.1_snapshot_", "").replace(".zip", ""))
    )
    for p in a1_1_snaps:
        pool.add(p, is_1v1_model=True)
    if a1_1_snaps:
