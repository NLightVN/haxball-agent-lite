"""
train_a1.0.py — Early Curriculum Training (Phase A1.1_0).
...
class A01Callback(BaseCallback):
    def __init__(self, model_dir, envs, save_freq, verbose=1):
        ...
        self.best_path = os.path.join(model_dir, "a1_0_best")
        self._a1_0_ball_goal_record_reward_total = 0.0
        self._a1_0_episode_count = 0
    ...
    def _on_step(self) -> bool:
        ...
        for i, done in enumerate(dones):
            if done and i < len(self.envs.envs):
                ...
                self._a1_0_ball_goal_record_reward_total += float(getattr(env, "_a1_0_record_ball_to_goal_reward_total", 0.0))
                self._a1_0_episode_count += 1

        if self._a1_0_episode_count > 0:
            self.logger.record(
                "a1.1_0/ball_goal_record_reward_mean",
                self._a1_0_ball_goal_record_reward_total / self._a1_0_episode_count,
            )
            if infos:
                last_info = infos[-1]
                if isinstance(last_info, dict):
                    self.logger.record(
                        "a1.1_0/best_ball_dist_to_goal",
                        float(last_info.get("a1.1_0/best_ball_dist_to_goal", 0.0)),
                    )
                    self.logger.record(
                        "a1.1_0/cur_ball_dist_to_goal",
                        float(last_info.get("a1.1_0/cur_ball_dist_to_goal", 0.0)),
                    )
                    self.logger.record(
                        "a1.1_0/steps_since_ball_record",
                        float(last_info.get("a1.1_0/steps_since_ball_record", 0.0)),
                    )
        ...
        if self.n_calls % self.save_freq == 0:
            snapshot_name = f"a1_0_snapshot_{self.num_timesteps}"
            path = os.path.join(self.model_dir, "a1_0_checkpoints", f"{snapshot_name}.zip")
            self.model.save(path)
            if self.verbose:
                log.info(f"[A1.1_0] Saved snapshot to {path}")
...
if __name__ == "__main__":
    ...
    os.makedirs("models/1v1 oriented/a1_0_checkpoints", exist_ok=True)

    def make_env():
        return HaxballCurriculumEnv(phase='A1.1_0', n_agents=1)

    vec_env = DummyVecEnv([make_env for _ in range(N_ENVS)])
    vec_env = VecMonitor(vec_env)

    if args.resume:
        log.info(f"[A1.1_0] Resuming from {args.resume}")
    ...
    log.info(f"[A1.1_0] Starting — {N_ENVS} envs, {remaining_steps:,} steps to go")
    ...
    model.save("models/1v1 oriented/a1_0_final")
    log.info("[A1.1_0] Done -> models/1v1 oriented/a1_0_final.zip")
