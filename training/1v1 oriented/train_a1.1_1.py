...
class SelfPlayCallback(BaseCallback):
    def __init__(self, opponent_manager, target_reward, model_dir, envs, verbose=1):
        ...
        self.best_path = os.path.join(model_dir, "a1_1_best")
    ...
    def _on_step(self) -> bool:
        ...
        while self.next_snapshot_at <= self.num_timesteps:
            ...
            path = os.path.join(self.model_dir, "a1_1_checkpoints", f"{snapshot_name}.zip")
...
def parse_args():
    ...
    p.add_argument("--a0-model", default="models/1v1 oriented/a1_0_checkpoints/a1_0_snapshot_5000000.zip", help="Path to initial A1.1_0 model")
...
if __name__ == "__main__":
    ...
    os.makedirs("models/1v1 oriented/a1_1_checkpoints", exist_ok=True)

    # 1. Initialize Opponent Manager
    opponent_manager = OpponentManager(args.a0_model)

    def make_env():
        return SelfPlayEnv(opponent_manager=opponent_manager, phase='A1.1_1')
    ...
    if is_resume:
        ckpt_dir = "models/1v1 oriented/a1_1_checkpoints"
        existing = sorted(
    ...
    model.save("models/1v1 oriented/a1_1_final")
    log.info("[SelfPlay] Done -> models/1v1 oriented/a1_1_final.zip")
